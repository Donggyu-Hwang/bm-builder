...
2. **데이터 보안 (Data Security):**
   - 전송 중 암호화 (TLS 1.3)
   - 저장 데이터 암호화 (AES-256, Supabase 기본 제공)
   - 개인정보보호법 준수
...
**낮은 리스크 (Low Risk):**
1. **OAuth 2.0 통합** - Supabase Auth로 간단 구현
...
- ✅ Supabase 기반 사용자 인증 (Google OAuth)
...
**Option 2: Next.js Supabase Starter** (Full-stack)
- **장점**: Full-stack 통합, Supabase 사전 구성
...
**Database:**
- **선택**: Supabase (PostgreSQL 15 + pgvector 0.5.0)
- **버전**: @supabase/supabase-js@2.90.1
- **호스팅**: Supabase Cloud (MVP), AWS RDS (Post-MVP 마이그레이션)
- **이유**: Auth 내장, RLS, pgvector 지원, 무료 500MB 플랜

**Data Modeling Approach:**
- **선택**: Supabase Client 직접 사용
- **ORM 없음**: Prisma 제외 (Supabase Client 사용)
...
1. **Supabase 프로젝트 설정** (Database + Auth)
...
- **Supabase → 프론트/백엔드**: Auth, RLS, Realtime 기능 공유
...
- Database: Supabase (PostgreSQL 15, pgvector 0.5.0), @supabase/supabase-js 2.90.1
...
# 4. Database 마이그레이션 (backend/supabase/migrations/)
CREATE EXTENSION IF NOT EXISTS vector;

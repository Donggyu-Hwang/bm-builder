...
router.get(
  ...
  async (req: any, res) => {
    ...
    // Delete existing token for this user first, then insert new one
    ...
    res.redirect(
      `${process.env.CORS_ALLOWED_ORIGINS?.split(',')[0] || 'http://localhost:5173'}/dashboard`
    );
  }
);
...
router.post('/refresh', async (req: any, res: Response) => {
  ...
  const {
    rows: [user],
  } = await pool.query('SELECT id, email, full_name, avatar_url FROM profiles WHERE id = $1', [
    tokenRow.user_id,
  ]);
...
router.post('/welcome-shown', async (req: any, res: Response) => {
  const accessToken = req.cookies.access_token;
  ...
  try {
    await pool.query('UPDATE profiles SET welcome_shown = TRUE WHERE id = $1', [payload.userId]);

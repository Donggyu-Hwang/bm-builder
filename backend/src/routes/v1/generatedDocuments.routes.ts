...
router.get('/', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const sortBy = req.query.sort_by as 'created_at' | 'title' | 'created_at_desc' | 'created_at_asc' | undefined;
 ...
 */
router.get('/:id/preview', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({
 ...
 */
router.get('/:id/download/pdf', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({
 ...
 */
router.get('/:id/download/ppt', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({

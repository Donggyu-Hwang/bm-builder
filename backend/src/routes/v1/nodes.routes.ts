 ...
 */
router.put('/documents/:documentId/nodes', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
 ...
 */
router.patch('/documents/:documentId/nodes/:nodeId', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({
 ...
 */
router.delete('/documents/:documentId/nodes/:nodeId', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({
    ...
    // Save to database
    await pool.query(
      'UPDATE documents SET nodes = $1, updated_at = NOW() WHERE id = $2',
      [JSON.stringify(filteredNodes), documentId]
    );
 ...
 * Duplicate a node
 */
router.post('/documents/:documentId/nodes/:nodeId/duplicate', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({

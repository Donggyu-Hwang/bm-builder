 ...
 */
router.post('/:figureId/regenerate', requireAuth, async (req: Request, res: Response) => {
  try {
    const figureId = req.params.figureId;
    if (!figureId) {
      return res.status(400).json({
        success: false,
        error: { code: 'INVALID_ID', message: 'Figure ID is required' },
      });
    }
 ...
 * Delete a figure
 */
router.delete('/:figureId', requireAuth, async (req: Request, res: Response) => {
  try {
    const figureId = req.params.figureId;
    if (!figureId) {
      return res.status(400).json({

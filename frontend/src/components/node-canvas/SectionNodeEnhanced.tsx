/**
 * Enhanced Section Node Component
 * Represents a document section with visual feedback for selection and drag
...
export default function SectionNode({ data, selected }: NodeProps) {
  ...
  const backgroundColor = data.color || undefined;
  const borderColor = selected ? 'border-purple-600' : '';

  return (
    <div
      className={`px-6 py-4 shadow-md rounded-lg border-2 transition-all duration-200 min-w-[200px] cursor-move hover:shadow-lg ${
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        {data.icon && (
          <div className="text-xs opacity-70">
            <span className="inline-block w-6 h-6 bg-white/50 rounded flex items-center justify-center">
              {data.icon === 'file-text' && '📄'}
              {data.icon === 'image' && '🖼️'}
              {data.icon === 'code' && '💻'}
              {data.icon === 'chart' && '📊'}
